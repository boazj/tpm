...
clone() {
  local plugin="$1"
  local branch="$2"
  local path="$(plugin_path_helper "$plugin")"
  if [ -n "$branch" ]; then
    cd "$(tpm_path)" &&
      GIT_TERMINAL_PROMPT=0 git clone -b "$branch" --depth=1 --single-branch --shallow-submodules --recursive "$plugin" "$path" >/dev/null 2>&1
  else
    cd "$(tpm_path)" &&
      GIT_TERMINAL_PROMPT=0 git clone --depth=1 --single-branch --shallow-submodules --recursive "$plugin" "$path" >/dev/null 2>&1
  fi
}
